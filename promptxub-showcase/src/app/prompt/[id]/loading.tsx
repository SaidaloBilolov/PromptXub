  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-slate-100 flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 flex-1 w-full">
        ...
        {/* Recommended Prompts Skeleton */}
        <section className="space-y-6 pt-4">
          <div className="space-y-2 border-b border-slate-800 pb-4">
            <div className="h-4 w-36 rounded bg-slate-800/80 animate-pulse" />
            <div className="h-7 w-64 rounded-xl bg-slate-800/80 animate-pulse" />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-slate-900/60 border border-slate-800/80 rounded-2xl overflow-hidden space-y-3 p-4">
                <div className="aspect-video w-full rounded-xl bg-slate-800/70 animate-pulse" />
                <div className="h-4 w-3/4 rounded bg-slate-800/70 animate-pulse" />
                <div className="h-8 w-full rounded-lg bg-slate-950/70 animate-pulse" />
                <div className="flex justify-between items-center pt-2">
                  <div className="h-4 w-20 rounded bg-slate-800/70 animate-pulse" />
                  <div className="h-8 w-24 rounded-xl bg-slate-800/70 animate-pulse" />
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Sticky Bottom Mobile Bar Skeleton */}
      <div className="fixed bottom-0 inset-x-0 p-3 bg-slate-950/95 border-t border-slate-800 backdrop-blur-xl z-50 sm:hidden flex items-center gap-2">
        <div className="h-12 w-12 rounded-2xl bg-slate-900 border border-slate-800 animate-pulse" />
        <div className="h-12 flex-1 rounded-2xl bg-purple-900/50 animate-pulse" />
        <div className="h-12 w-12 rounded-2xl bg-slate-900 border border-slate-800 animate-pulse" />
      </div>
    </div>
  );
