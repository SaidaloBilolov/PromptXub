...
import { X, Sparkles, Mail, ArrowRight, ShieldCheck, CheckCircle2, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      ...
      <div className="relative w-full max-w-md bg-[#0F172A] border border-slate-700/90 rounded-3xl p-6 sm:p-8 shadow-2xl z-10 overflow-hidden space-y-6">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-full bg-slate-900/80 text-slate-400 hover:text-white border border-slate-700/80 transition hover:scale-105 active:scale-95 cursor-pointer z-20"
        ...
        {!isEmailSent ? (
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full py-3 px-4 rounded-xl font-bold text-xs sm:text-sm bg-slate-900 hover:bg-slate-800 text-slate-100 border border-slate-700/80 flex items-center justify-center gap-3 transition active:scale-[0.99] shadow-md cursor-pointer disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={loadingProvider !== null}
              className="w-full py-3 px-4 rounded-xl font-bold text-xs sm:text-sm bg-white hover:bg-slate-100 text-black flex items-center justify-center gap-3 transition active:scale-[0.99] shadow-md cursor-pointer disabled:opacity-50"
            >
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-2">
              ...
              <button
                type="submit"
                disabled={loadingProvider !== null}
                className="w-full py-3 rounded-xl font-bold text-xs sm:text-sm bg-gradient-to-r from-purple-600 to-indigo-600 hover:opacity-95 text-white flex items-center justify-center gap-2 shadow-lg shadow-purple-600/30 active:scale-[0.99] cursor-pointer disabled:opacity-50"
              >
          ...
          <div className="text-center py-4 space-y-3">
            ...
            <p className="text-xs text-slate-300 max-w-xs mx-auto">
              We sent an instant magic sign-in link to <span className="text-cyan-400 font-semibold">{email}</span>.
            </p>
            <button
              onClick={() => {
        ...
        <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-400">
          ...
          <button onClick={onClose} className="hover:text-slate-200 underline cursor-pointer">
            Browse freely first
          </button>
        </div>
