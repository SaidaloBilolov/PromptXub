...
export const PromptDetailView: React.FC<PromptDetailViewProps> = ({
  ...
  const handleCopyPrompt = async () => {
    try {
      ...
      showToast("Promptdan nusxa olindi! 🎉");
  ...
  const handleCopyNegative = async () => {
    ...
    try {
      ...
      showToast("Negative promptdan nusxa olindi!");
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleToggleSave = () => {
    ...
    setSaved(isNowSaved);
    if (isNowSaved) {
      showToast(`Kutubxonangizga saqlandi! 🔖`);
    } else {
  ...
  const handleCardCopy = async (cardPrompt: Prompt, e: React.MouseEvent) => {
    ...
    try {
      ...
      showToast(`Nusxalandi: "${cardPrompt.title.substring(0, 20)}..."`);
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-slate-100 selection:bg-purple-500 selection:text-white flex flex-col pb-24 sm:pb-16">
      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-800/80 bg-[#0F172A]/90 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          >
            ...
            <span className="hidden sm:inline">Showcase'ga qaytish</span>
            <span className="sm:hidden text-xs">Orqaga</span>
          </Link>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              ...
              title={saved ? 'Kutubxonada saqlangan' : 'Kutubxonaga saqlash'}
            >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 flex-1 w-full space-y-12">
          ...
          <div className="w-full lg:w-1/2 p-5 sm:p-8 lg:p-10 flex flex-col justify-between space-y-6">
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-purple-400 uppercase tracking-wider flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5" /> Prompt Matni
                  </span>
                ...
                {prompt.promptText.length > 180 && (
                  ...
                  >
                    <span>{isExpanded ? "Qisqartirish" : "To'liq promptni ko'rish"}</span>
              ...
              <div className="grid grid-cols-3 gap-3 p-3.5 rounded-2xl bg-slate-950/40 border border-slate-800/60 text-xs">
                <div>
                  <span className="text-slate-500 block mb-0.5 text-[11px]">Kategoriya</span>
                  <span className="font-semibold text-slate-200 truncate block">
                    {prompt.category?.name || 'General AI'}
                  </span>
                </div>
                <div>
                  <span className="text-slate-500 block mb-0.5 text-[11px]">Nusxalar</span>
                ...
                <div>
                  <span className="text-slate-500 block mb-0.5 text-[11px]">Ko'rishlar</span>
              ...
              >
                {copied ? (
                  <>
                    ...
                    <span>Prompt Nusxalandi!</span>
                  </>
                ) : (
                  <>
                    <Copy className="w-4 h-4" />
                    <span>Promptdan Nusxa Olish</span>
                  </>
                )}
              </button>
        ...
        {/* ========================================================================= */}
        {/* RECOMMENDED PROMPTS SECTION ("Tavsiya etilgan AI Promptlar") */}
        {/* ========================================================================= */}
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 border-b border-slate-800 pb-4">
              <div>
                ...
                <h2 className="text-xl sm:text-2xl font-extrabold text-white tracking-tight">
                  Tavsiya etilgan AI Promptlar
                </h2>
                <p className="text-xs sm:text-sm text-slate-400 mt-1">
                  {prompt.category?.name || 'Showcase'} yo'nalishidagi eng ommabop va mashhur promptlar
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
              {recommendedPrompts.map((rec) => {
                ...
                return (
                  <div
                    key={rec.id}
                    className="group bg-slate-900/60 border border-slate-800/80 hover:border-purple-500/40 rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-purple-600/10 flex flex-col"
                  >
                    {/* Media Thumbnail */}
                    <Link
                      href={`/prompt/${rec.id}`}
                    ...
                    >
                      <img
                        src={getOptimizedMediaUrl(rec.thumbnailUrl || rec.mediaUrl, { width: 600 })}
                        alt={rec.title}
                      ...
                      <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 z-10">
                        <span className="px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider bg-slate-950/80 text-purple-300 border border-purple-800/60 backdrop-blur-md">
                          {rec.aiModel}
                        </span>
                        {rec.contentType === 'VIDEO' && (
                          <span className="px-1.5 py-0.5 rounded-md text-[10px] font-semibold bg-cyan-950/80 text-cyan-300 border border-cyan-800/60 backdrop-blur-md flex items-center gap-1">
                      ...
                      {/* Aspect ratio */}
                      {rec.aspectRatio && (
                        <span className="absolute top-2.5 right-2.5 px-2 py-0.5 rounded-md text-[10px] font-mono bg-slate-950/80 text-slate-300 border border-slate-800 backdrop-blur-md">
                          {rec.aspectRatio}
                        </span>
                      )}
                    </Link>
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                      <div className="space-y-1.5">
                        <Link
                          href={`/prompt/${rec.id}`}
                          className="font-bold text-sm text-white group-hover:text-purple-300 transition line-clamp-1 block"
                        >
                          {rec.title}
                        </Link>
                        <p className="text-xs text-slate-400 line-clamp-2 font-mono bg-slate-950/50 p-2 rounded-lg border border-slate-800/50">
                          {rec.promptText}
                        </p>
                      </div>

                      {/* Card Bottom: Stats + Quick Copy */}
      ...
      {/* ========================================================================= */}
      {/* STICKY BOTTOM MOBILE ACTION BAR (Max Visibility for Mobile Users) */}
      {/* ========================================================================= */}
      <div className="fixed bottom-0 inset-x-0 p-3 bg-slate-950/95 border-t border-slate-800/90 backdrop-blur-2xl z-50 sm:hidden flex items-center gap-2 shadow-2xl">
        <button
          ...
          aria-label="Kutubxonaga saqlash"
        >
