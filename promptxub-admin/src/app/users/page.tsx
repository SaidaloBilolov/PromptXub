...
export default function UsersPage() {
  ...
  const filteredUsers = userStats.usersList.filter((user) => {
    ...
    const matchesProvider =
      selectedProvider === 'ALL' || user.provider === selectedProvider;
    ...
    <div className="min-h-screen bg-[#0F172A] flex text-slate-100">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <AdminNavbar
          ...
          subtitle="Manage registered users, auth providers, and bookmarks"
        />
        ...
        <main className="p-8 space-y-8 flex-1 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-xl relative overflow-hidden">
              ...
              <div className="text-3xl font-extrabold text-white mb-2">{userStats.totalUsers.toLocaleString()}</div>
              <div className="text-xs text-purple-400 font-semibold">Active accounts across showcase</div>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-xl relative overflow-hidden">
              ...
              <div className="text-xs text-cyan-400 font-semibold">
                {Math.round((userStats.googleUsersCount / userStats.totalUsers) * 100)}% of total users
              </div>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-xl relative overflow-hidden">
              ...
              <div className="text-xs text-slate-400 font-semibold">
                {Math.round((userStats.appleUsersCount / userStats.totalUsers) * 100)}% iOS / macOS users
              </div>
            </div>
                    ...
                    <tr key={user.id} className="hover:bg-slate-800/40 transition">
                      ...
                      <td className="py-3.5 px-3 text-right font-bold text-purple-400">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <span>{user.savedPromptsCount}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-3.5 pr-2 text-right">
                        <button
                          ...
                          className={`px-3 py-1 rounded-lg text-xs font-semibold border transition active:scale-95 ${
                            user.enabled
